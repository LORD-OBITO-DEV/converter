...
export default function Signup(){
  ...
  async function submit(e){
    ...
    try{
      ...
      const token = r.data.token; localStorage.setItem('token', token); setAuth(token);
      navigate('/');
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded shadow">
      ...
      <form onSubmit={submit} className="space-y-3">
        <input className="w-full p-2 border rounded" placeholder="Nom" value={name} onChange={e=>setName(e.target.value)} />
        ...
        <button className="w-full py-2 bg-black text-white rounded">S'inscrire</button>
      </form>
    </div>
  )
}
